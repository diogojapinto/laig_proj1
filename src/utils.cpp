...
float* getNewellsMethodSecondForm(std::vector<float> x, std::vector<float> y, std::vector<float> z, float x_center,float y_center,float z_center)
{
	if (x.size() != y.size() || x.size() != z.size())
		return NULL;
	...
	for (unsigned int i = 0; i < nVertices; i++)
	{
		...
		float Vx = (x[indiceNextVertex] - x_center), Vy = (y[indiceNextVertex] - y_center), Vz = (z[indiceNextVertex] - z_center);
...
float* getNormalsAlternative(std::vector<float> x, std::vector<float> y, std::vector<float> z, float x_center,float y_center,float z_center)
{
	if (x.size() != y.size() || x.size() != z.size())
		return NULL;
