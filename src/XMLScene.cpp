...
bool XMLScene::parseNode(TiXmlElement *curr_node, vector<string> nodes_processed) {
	...
	while ((child = (TiXmlElement *) children->IterateChildren(child))) {
		...
		if (strcmp(child_type, "rectangle") == 0) {
			...
			if (find(nodes_processed.begin(), nodes_processed.end(), next_node_id) != nodes_processed.end()) {
