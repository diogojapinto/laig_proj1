...
void XMLScene::loadFile() {
	...
	if ((animationsElement = yafElement->FirstChildElement("animations"))
			== NULL) { //block animations is not mandatory??? our choice
...
bool XMLScene::parseAppearences() {
	...
	if ((app = appearencesElement->FirstChildElement("appearance")) != NULL) {
		...
		do {
			...
			new_app->setSpecular(app_spec_r, app_spec_g, app_spec_b,
					app_spec_a);
			...
			if (strdup(app_text_ref, app->Attribute("textureref")) != NULL) {
				if (strcmp(app_text_ref, "") != 0) {
					...
					new_app->setTextProp(app_text_ref, app_text_len_s,
							app_text_len_t);
				} else {
...
bool XMLScene::parseNode(TiXmlElement *curr_node, bool is_inside_dl) {
	...
	while ((child = (TiXmlElement *) children->IterateChildren(child))) {
		...
		if (strcmp(child_type, "rectangle") == 0) {
			...
			MyTriangle *tri = new MyTriangle(x1, y1, z1, x2, y2, z2, x3, y3,
					z3);
			n->addPrimitive(tri);
			...
			MyTorus *tor = new MyTorus(tor_inner, tor_out, tor_slices,
					tor_loops);
			n->addPrimitive(tor);
